...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, model_b=None):
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if model_b is not None:
            ...
            with torch.no_grad():
                model_b_render = render(viewpoint_cam, model_b, pipe, background)["render"]
            gt_image = model_b_render
        else:
...
def load_pretrained_model(model_path, dataset, opt):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_b_path", type=str, default=None, help="Path to pretrained model b for rendering GT")
    ...
    model_b = None
    if args.model_b_path:  # 添加新命令行参数
        model_b = load_pretrained_model(args.model_b_path)
    
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, model_b=model_b)
